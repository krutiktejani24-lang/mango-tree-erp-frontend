...
/**
 * Shared form for creating and editing an invoice.
 * @param {object} defaultValues - initial form values (create passes blank defaults, edit passes the loaded invoice)
 * @param {(payload: object) => Promise<void>} onSubmit - receives the built invoice payload; caller does the actual API call
 * @param {string} submitLabel
 * @param {string} submittingLabel
 * @param {boolean} showPaymentStatus - hide on edit (payment is handled separately via Record Payment)
 */
export default function InvoiceForm({ defaultValues, onSubmit, submitLabel, submittingLabel, showPaymentStatus = true }) {
  ...
  const { register, control, handleSubmit, watch, setValue, formState: { isSubmitting } } = useForm({ defaultValues });

  React.useEffect(() => {
    if (settings && defaultValues.gst_percentage == null) {
  ...
  async function submit(values) {
    ...
    if (showPaymentStatus) payload.payment_status = values.payment_status;

    await onSubmit(payload);
  }

  return (
    <form onSubmit={handleSubmit(submit)} className="grid lg:grid-cols-3 gap-6">
